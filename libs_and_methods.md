...
b = a.split()    # Делит строку по определённому символу и возвращет список строк, по умолчанию пробел
print(b)
>>> ['Кривизна', 'материи']
...
Буква Z превращается в букву А. Игра продолжается, пока в строке возможны превращения одних букв в другие. 
...
2. Найдите последнее из любимых блюд Вероники, а также посчитайте, сколько денег у неё есть в сумме:
```py
a = {'Вероника': {'Дата рождения': '13.09.1984',
                  'Любимая еда': ['Макарошки с сыром', 'Салат Оливье', 'Пицца Пепперони'],
                  'Баланс': {'Наличными': 1200.50, 'Мелочью': 53.80, 'В банке': 10712.00}
...
```
3. Сделайте обработчик команд! Ну, пока что, одной команды... С клавиаутры пишется `square n`, где `n` — это число, которое нужно возвести в квадрат. Вам нужно понять, что написана именно эта команда, а не что-то ещё, при помощи `startswith()`, затем отделить число от команды и возвести его в квадрат, что и будет выводом в консоль!  
...
# Если у вас не работает / в пути, попробуйте заменить его на два обратных слеша: 'Картиночки\\Тессеракт.png'
...
t = time.strftime("%d.%m.%Y, %H:%M:%S", time.localtime())    # а так можно вывести строку любого формата вообще
print(t)
>>> 25.10.2021, 11:32:54
```
...
### Задачи ❓
Не забудьте `import`!
4. Вам нужно достать случайную картинку из папки `pics`. Вот незадача: в папке лежат не только картинки, кто-то напихал туда чёрт знает чего! Презентации, таблицы и даже установщик браузера Амиго! Благо, есть метод `endswith()`, а все картинки у нас в фотмате `.png`. Получите список картинок и выведите случайную.
5. Нет, ну если владельца презентации нам ещё предстоит найти, браузер Амиго мы точно терпеть не можем! Нужно его удалить. Предлагаю для этого найти в папке `pics` такой файл, который содержит `Amigo`. Он может быть и с маленькой буквы, и в середине, так что используйте такой код: `if 'amigo' in i.lower()`. А ещё не забывайте, что при удалении нужно указать не только название файла, но и весь путь вместе с папкой `pics`.
6. Который час? Выведите строку вида `"10ч15м"`, используя `time.strftime()`.
