...
### Задачи
Давайте решим те же самые уравнения, но будем вводить `x` с клавиатуры:
```
3 * (x + 4) / 5 + 3 / 8
y = -10 + 4 * x + 0.5 * x ** 2
```
Дано время в секундах: `s = 560`. Переведите это в минуты и выведите строку вида `9м20с`.

## Условия и операторы сравнения
Операторы сравнения мы знаем из математики. Вот такой вид они имеют в Питоне: `>`, `>=`, `<`, `<=`, `==`
Предположим, нам нужно выполнять какой-то код, только если выполняется определённое условие. Все строки этого кода нужно как-то выделить. В других языках для этого используются фигурные скобки, а нам нужно выделить их табуляцией. Она ставится либо клавишей Tab, либо 4 пробелами:
```py
a = int(input())
if a >= 9:
