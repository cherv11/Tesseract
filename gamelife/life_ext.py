    ...
    for i in range(MAP_SIZE[0]):
        for j in range(MAP_SIZE[1]):
            if not lastplaymap[i][j]:
                if check_neighbours(i, j) in (3,):
                    playmap[i][j] = 1
            elif not 2 <= check_neighbours(i, j) <= 3:
                playmap[i][j] = 0
    for i in playmap:
        if i[0]:
            playmap = np.concatenate((np.zeros((playmap.shape[0], 1), 'int'), playmap), axis=1)
            MAP_SIZE = playmap.shape
            TILE_SIZE = min([FULL_WINDOW[1] // MAP_SIZE[0], FULL_WINDOW[0] // MAP_SIZE[1]])
            break
    for i in playmap:
        if i[-1]:
            playmap = np.concatenate((playmap, np.zeros((playmap.shape[0], 1), 'int')), axis=1)
            MAP_SIZE = playmap.shape
            TILE_SIZE = min([FULL_WINDOW[1] // MAP_SIZE[0], FULL_WINDOW[0] // MAP_SIZE[1]])
            break
    for i in playmap[0]:
        if i:
            playmap = np.concatenate((np.zeros((1, playmap.shape[1]), 'int'), playmap), axis=0)
            MAP_SIZE = playmap.shape
            TILE_SIZE = min([FULL_WINDOW[1] // MAP_SIZE[0], FULL_WINDOW[0] // MAP_SIZE[1]])
            break
    for i in playmap[-1]:
        if i:
            playmap = np.concatenate((playmap, np.zeros((1, playmap.shape[1]), 'int')), axis=0)
            MAP_SIZE = playmap.shape
            TILE_SIZE = min([FULL_WINDOW[1] // MAP_SIZE[0], FULL_WINDOW[0] // MAP_SIZE[1]])
            break
